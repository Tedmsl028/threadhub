...
if(isset($_GET['edit'])){
  ...
  if(isset($_GET['delete_image'])){
    ...
    $db->query("UPDATE products SET image = '{$imageString}' WHERE product_id = '$edit_id'");
    header('Location: products.php?edit='.$edit_id);
  }
  $category = ((isset($_POST['child']) && $_POST['child'] != '')?sanitize($_POST['child']):$product['categories']);
  $title = ((isset($_POST['title']) && $_POST['title'] != '')?sanitize($_POST['title']):$product['title']);
  $brand = ((isset($_POST['brand']) && $_POST['brand'] != '')?sanitize($_POST['brand']):$product['brand_name']);
  ...
  $photoCount = count($_FILES['photo']['name']);
   if ($photoCount > 0) {
     for($i = 0;$i<$photoCount;$i++){
      ...
      $dbpath .= '/threadhub/images/products/'.$uploadName;
      if ($mimeType != 'image') {
  ...
  if(!empty($errors)){
    ...
    //upload file and insert into database
      for($i=0;$i<$photoCount;$i++){
        ...
        $insertSql = "INSERT INTO products (`product_id`, `title`, `price`, `list_price`, `brand`, `categories`, `sizes`, `image`, `description`)
    ...
    <div class="form-group col-md-3">
      ...
      <select class="form-control" id="brand" name="brand">
        ...
        <?php while($b = mysqli_fetch_assoc($brandQuery)): ?>
          <option value="<?=$b['brand_id'];?>"<?=(($brand == $b['brand_id'])?' selected':'');?>><?=$b['brand_name'];?></option>
        <?php endwhile; ?>
      </select>
    </div>
    <div class="form-group col-md-3">
  ...
  <tbody>
    <?php while($product = mysqli_fetch_assoc($presults)):
      ...
      <tr>
        ...
        <td><?=$category;?></td>
        <td><a href="products.php?featured=<?=(($product['featured'] == 0)?'1':'0');?>&product_id=<?=$product['product_id'];?>" class="btn btn-xs btn-default">
