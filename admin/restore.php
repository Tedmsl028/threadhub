...
?>
<h2 class="text-center">Restore Products</h2>
...
<table class="table table-bordered table-condensed talbe-striped">
 ...
 <tbody>
  ...
  ?>
    <tr>
     <td>
      <a href="restore.php?restore=<?php echo $product['product_id']; ?>" class="btn btn-xs btn-primary"><span class="glyphicon glyphicon-refresh"></span></a>
     </td> <!--EDIT OR REMOVE -->
     <td><?php echo $product['title']; ?></td> <!-- TITLE -->
     <td><?php echo money($product['price']); ?></td> <!-- PRICE -->
     <td><?php echo $category; ?></td> <!-- Categories -->
     <td><a href="products.php?featured=<?php echo (($product['featured'] == 0)?'1':'0'); ?>&id=<?php echo $product['product_id']; ?>" class="btn btn-xs btn-<?php echo (($product['featured'] == 1)?'warning':'success'); ?>">
